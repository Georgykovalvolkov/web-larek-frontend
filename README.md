...
Конструктор:
- constructor(container: HTMLFormElement, events: IEvents) - Конструктор принимает два параметра:
container: контейнер формы, где будут происходить все манипуляции.
...
Предназначен для реализации модального окна с формой содержащей кнопку выбора способа оплаты и поля ввода адреса. 
...
#### Класс ModalWithForm
 Предназначен для реализации модального окна с формой содержащей поля ввода. При сабмите инициирует событие передавая в него объект с данными из полей ввода формы.
Предоставляет методы для отображения ошибок и управления активностью кнопки продолжения оформления заказа.\
Поля класса:
